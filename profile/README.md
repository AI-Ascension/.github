...
**Curious player or observer.** Nothing here touches a game yet. You can watch exactly what is tested today and what is only proposed: a stale lease is denied before any transport call, and the stamp reads `unverified` for live play. [Run the proof](https://ai-ascension.github.io/proof.html), then read [what is tested versus proposed](https://ai-ascension.github.io/evidence.html). If a label confuses you, [report it](https://github.com/AI-Ascension/.github/issues/new?template=evidence.yml).

**Rust or systems contributor.** You do not need all six repositories. Start with one boundary, the gateway lease fence, and one bounded task. [Run the cargo recipe](https://ai-ascension.github.io/recipes.html): it prints the same trace as the browser replay, with the exact test names and source lines. Then take a `first-task` issue and [submit a fence-case fixture](https://github.com/AI-Ascension/.github/issues/new?template=proof-recipe.yml).

**AI, MCP, or automation builder.** In the in-memory tests, one MCP tool call maps to exactly one gateway request path, and malformed frames are rejected before the gateway is reached; a live transport is `proposed`. Read the [architecture](https://ai-ascension.github.io/architecture.html), run the recipe without any credentials, then sketch an adapter example against the tool schema and [file a contract observation](https://github.com/AI-Ascension/.github/issues/new?template=contract-observation.yml).

**Maintainer or security operator.** Authority stays with the game host. The gateway denies stale epochs, wrong instances, and oversize bodies before transport and fails closed on process, readiness, and transport faults. Read the [evidence page](https://ai-ascension.github.io/evidence.html) and [SECURITY.md](https://github.com/AI-Ascension/.github/blob/main/SECURITY.md), verify the claim-to-test mapping yourself, then send a security note privately or a failure-handling observation.

## The ascent

Read from the bottom up. A request from a model descends through each tier toward the game host, and each tier can refuse it. Every "Owns" cell is the repository's approved description.
...
| game host | not a repository | The game itself. No game files are stored, read, or distributed. | Nothing touches it. Live path: `proposed`; compatibility: `unverified`. |
...
Every repository is Rust, MIT-licensed, and contains no game files. Full map: [repositories](https://ai-ascension.github.io/repositories.html).
