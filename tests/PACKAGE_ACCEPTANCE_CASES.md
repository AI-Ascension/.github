...
initial independent review status is retained here while the package follow-up
closes focused gaps. Final dispositions and evidence are maintained in the
[acceptance matrix](ACCEPTANCE_MATRIX.md). `PARTIAL` and `UNVERIFIED` are not passes.

| ID | Scenario | Required observation | Initial review |
| --- | --- | --- | --- |
| INV-01 | Accessible repository list expands or paginates | Every accessible entry is inventoried once; missing access is explicit. | PARTIAL |
| INV-02 | Default branch is not main | Actual default branch/commit is used; branch is not changed. | PASS-review |
| INV-03 | Blob/tree identity provided as a head pin | Reject as commit evidence until resolved to an actual commit. | PARTIAL |
...
| RUS-02 | Member locally weakens a mandatory lint | Detect or require a valid exact approved exception. | PARTIAL |
...
| RUS-06 | Unsafe outside approved boundary | Fail; an approved FFI boundary remains compilable with checked scope. | PARTIAL |
| RUS-07 | Dangerous or mutually incompatible feature combination | No live effect; explicit supported matrix and truthful exclusion. | PARTIAL |
| RUS-08 | Documentation example is broken | Explicit documentation-test lane fails. | PARTIAL |
...
| POL-04 | Unreadable or malformed policy/config | Fail explicitly; never run with empty defaults. | PARTIAL |
...
| CON-02 | Wire/ABI/environment spelling is normalized | Compatibility check rejects an unapproved external-name change. | PARTIAL |
...
| NET-02 | Callback after shutdown or ABI version/length mismatch | Source-linked negative test rejects or contains invalid access. | PARTIAL |
| NET-03 | Proprietary host dependency unavailable | Report host lane not-run/blocked, preserve source-only tests. | UNVERIFIED |
...
| PHP-05 | Corrupt store, failed open/lock/write/replacement | No false persistence success or destructive empty-state overwrite. | PARTIAL |
...
| PHP-08 | Production PHP differs from PHPUnit runtime | Separate minima are truthful and tested without unapproved upgrade. | PARTIAL |
...
| WEB-02 | HTML receives untrusted content | Content remains data; no unintended executable insertion. | PARTIAL |
| OPS-01 | Dotenv contains shell metacharacters | Parsed as data; no command execution. | PASS-local |
| OPS-02 | Native subprocess fails on Bash/PowerShell | Failure propagates; owned cleanup is bounded and scoped. | PARTIAL |
| OPS-03 | Dry run with existing data/config | No service/data/credential mutation or destructive cleanup. | PARTIAL |
| OPS-04 | Host-public publish versus internal container bind | Correct distinction; unapproved exposure fails. | PASS-local |
| CI-01 | Missing tool/target or zero selected tests | Not a successful required check. | PARTIAL |
| CI-02 | Matrix job skipped/cancelled/failed | Aggregate cannot falsely report all mandatory checks passed. | PASS-local |
| CI-03 | PR edits policy to disable its own required check | Protected baseline/owner review detects the enforcement change. | PARTIAL |
| CI-04 | Untrusted PR attempts privileged execution | No secret-bearing/privileged code execution. | PARTIAL |
| CI-05 | Public reports include rendered secret configuration | Artifact/log privacy test rejects publication. | PARTIAL |
...
| REP-02 | Warm/cold cache or supported platform changes | Tool/target provenance and actual prerequisites remain explicit. | PARTIAL |
