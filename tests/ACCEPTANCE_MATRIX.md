...
| Frozen contract/artifact bytes remain unchanged | Native checksum gates and original-commit byte audit | 441 selected existing schema/artifact/conformance/fixture/recipe/evidence/checksum files compared; zero changed or missing. |
| PHP origin/privacy/input correctness | Composer PHPUnit regressions, PHPStan and CS Fixer | 49 tests/121 assertions pass; synthetic origin, shape, bounds, error mapping and log-marker cases use no real subscriber data or mail. |
...
| Browser behavior is distinct from synthetic DOM tests | Node and Chromium suites in both sites | PHP site: 2 Node/3 browser cases; Pages: 10 Node/3 browser cases. Keyboard/focus/error/reduced-motion behavior checked in Chromium. |
...
| Requested orchestration ancestry is real | Actual execution metadata and native agent inventory | Eleven native descendants across waves used requested Luna/max, all at depth one. Depth-two/three tools were unavailable; no claim of the required three descendant layers. |
